...
//! dominates query latency — on a ~2.3k-file tree it is ~75% of an `rr at` —
//! so this isolates it from the process start, mmap fault-in, and covering
//! scan that an end-to-end timing folds together.
//!
//!   cargo bench --bench freshness
//!
//! Each count gets a throwaway temp tree of empty files. The walk does one
//! `stat` per path, so file *count* is the only variable that matters
//! (contents are irrelevant). The counts bracket the realistic range; 256 is
//! the parallel threshold, so every measured count takes the threaded path.
...
/// `stat` per path, newest wins, missing files contribute 0. Replicated here
/// as the baseline rather than exposed from the lib so the public API stays
/// minimal — the operation is fixed, so the two cannot drift in any way that
/// matters.
