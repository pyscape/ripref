//! Tuning bench for the freshness stat-walk's parallel constants. The
//! library's [`ripref::indexer::newest_mtime`] hard-codes `PARALLEL_THRESHOLD
//! = 256` (below that count it runs serial) and otherwise fans across every
//! core. Those two numbers were picked by reasoning, not measurement. This
//! bench measures them: where parallel first beats serial (the
//! empirically-correct threshold) and how the parallel reduction scales with
//! thread count.
//!
...
//! `indexer::newest_mtime`: serial is one `stat` per path, newest wins,
//! missing files contribute 0; parallel is `chunk = len.div_ceil(n_threads)`,
//! one `thread::scope` thread per chunk each running the serial max-reduce,
//! overall max. The operation is fixed, so the replicas cannot drift in any
//! way that matters -- but the measured crossover only applies to the real
//! function while they match it. If `newest_mtime`'s reduction changes, update
//! this bench in step. (`freshness.rs` makes the same argument for its serial
//! replica.)
//!
//! Caveats:
//!   - This is a MANUAL tuning bench. It is NOT wired into the CI
//!     bench-regression gate, which stays freshness + query. Run it by hand
//!     when retuning.
//!   - On Windows the %TEMP% stat cost is high and noisy (Defender taxes every
//!     metadata call), so the CROSSOVER POINT and the serial-vs-parallel
//!     RATIOS are the signal here, not the absolute microseconds. Run on Linux
//!     for clean absolutes and the real platform threshold: cheaper stats
//!     there shift the crossover higher, so the threshold is
//!     platform-dependent and the Linux run is the representative one.
...
/// Create `n` empty files under a fresh temp dir; return the dir (kept alive
/// by the caller for the bench's lifetime) and the relative paths to walk.
fn make_tree(n: usize) -> (std::path::PathBuf, Vec<String>) {
    let dir = std::env::temp_dir().join(format!(
        "rr-fresh-scale-bench-{}-{}",
        std::process::id(),
        n
    ));
...
/// The chunked parallel max-reduce with no threshold guard and an explicit
/// thread count. Mirrors `indexer::newest_mtime`'s reduction exactly, minus
/// the guard: `chunk = len.div_ceil(n_threads)`, one `thread::scope` thread
/// per chunk each running `newest_serial`, overall max. `n_threads` is clamped
/// to the path count so empty chunks are never spawned.
...
fn bench_threads(c: &mut Criterion) {
    ...
    // only adds scheduling contention), then drop the duplicates that cap
    // produces on small machines -- 4 and 8 both become 4 on a 4-core box --
    // so criterion never sees two benchmarks with the same id. THREAD_SWEEP is
    // ascending, so the capped values stay ordered and consecutive dedup
    // suffices.
    let mut sweep: Vec<usize> =
        THREAD_SWEEP.iter().map(|&t| t.min(all)).collect();
    sweep.dedup();
    for &t in &sweep {
        group.bench_with_input(
